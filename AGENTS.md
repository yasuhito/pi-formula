...
- ハーネスのテストは動作の結果を観察する。`scripts/verify-display` と `scripts/verify-extensions/` を検査するときは、実行した結果（終了コード、出力、キャプチャ）を確かめる。ソースを `readFileSync` で読んで文字列の出現や順序を検査する新しいテストは追加しない。ハーネスは「撮るまでを自動化し、合否は目で見る」道具なので、判定ロジックそのものへの投資は目的から外れる。
