...
## Scenario: 応答を一字一句確認してからキャプチャする

- When 実表示検証の検証契約を出力する
- Then コーパスモードでは応答一致をキャプチャより前に計画する

## Scenario: 実表示検証はコーパスモードだけを受け付ける

- Given 探索モードの引数を持つ実表示検証コマンドがある
- When 実表示検証コマンドを実行する
- Then 探索モードを使えずコーパスモードの使い方が表示される

## Scenario: 実表示検証とプロトコル層の検証の役割を出力する
...
## Scenario Outline: 末尾改行の有無にかかわらずコーパスモードの応答を受理する

- Given `<side>` だけが末尾改行を持つコーパスモードのセッション記録がある
- When 応答とコーパスの一致を検証する
- Then 末尾改行だけが異なる応答は受理される
...
## Scenario: 末尾改行以外の一文字差を報告する

- Given 末尾改行以外に一文字差があるコーパスモードのセッション記録がある
- When 応答とコーパスの一致を検証する
- Then 不一致の位置と正規化後の文字数が報告される

## Scenario Outline: 空白が異なる回帰入力を拒否する

- Given コーパスへ `<position>` の空白を加えた assistant のセッション記録がある
- When 応答とコーパスの一致を検証する
- Then 空白が異なる応答はキャプチャ前に拒否される
