...
## Scenario: 返した PNG データの変更をキャッシュから隔離する

- Given 画像経路を使う試験用の連携拡張がある
- When 返された PNG データを変更して同じ表示数式を再び描く
- Then 次の公開 PNG と通常の表示数式は壊れない
...
- When runtime を終了して別の利用者マクロを使う runtime を登録する
- Then 新しい runtime に描画と formula コマンドが一つずつ登録される
