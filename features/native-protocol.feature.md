...
## Scenario: native ツールがない環境ではストリーミング検査を skip する

- Given vt-pty がない環境がある
- When ストリーミング中のプロトコル検査を実行する
- Then ストリーミング中の検査を成功として skip したことが出力される

`@native-vt`
## Scenario: vt-pty が読み取りごとのプロトコル状態を記録する
...
`@native-vt`
## Scenario: 先行する tool 出力を保って未完了本文を差分描画する
