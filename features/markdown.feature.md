...
## Scenario: ストリーミング中の閉じた数式を画像で描く

- Given 画像経路で数式を描ける Pi がある
- When 閉じた数式まで届いたストリーミング本文を変換する
- Then 閉じた表示数式は画像経路で描かれる
...
## Scenario Outline: コロン付きラベルの再走査した金額の後ろにある表示数式を描く

- Given 画像経路で数式を描ける Pi がある
- When コロン付きラベル「<ラベル>」の再走査される金額と後続の表示数式を含む本文を変換する
- Then コロン付きラベルの金額が一度だけ残り画像経路で描かれる式は x = 1 だけになる
...
## Scenario: 再走査した金額と表示数式の後ろにあるシェルの $$ を保つ

- Given 画像経路で数式を描ける Pi がある
- When 再走査される金額と表示数式と末尾のシェルの $$ を変換する
- Then 金額とシェルは一度だけ残り画像経路で描かれる式は x = 1 だけになる
...
## Scenario: 数式でない $$ の後ろにあるラベル付き数値表示数式を描く

- Given 画像経路で数式を描ける Pi がある
- When 数式でない $$ と行内ラベル付きの数値表示数式を変換する
- Then 画像経路で描かれるラベル付きの式は 100 だけになる
...
## Scenario: 数値表示数式の後ろにある閉じていない $$ を保つ

- Given 画像経路で数式を描ける Pi がある
- When 数式でない $$ と数値表示数式と単一英字と閉じていない $$ を変換する
- Then 100 だけが画像になり単一英字と末尾区切りは残る
