...
const harness = fs.readFileSync(path.resolve(__dirname, '../scripts/verify-display'), 'utf8');
...
test('process group とウィンドウを止めてから headless 出力を削除する', () => {
  ...
  assert.ok(cleanup.indexOf('stop-display-process') < cleanup.indexOf('output remove'));
