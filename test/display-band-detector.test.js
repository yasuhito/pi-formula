...
function runPng(png, timeout = 5_000) {
  ...
  const result = spawnSync(process.execPath, [detector, ...detectorArgs, filename], {
...
test('ID 色の水平帯を座標付きで検出する', () => {
  const result = runPng(createPng(120, 80, (x, y) => {
...
test('途中で切れた scanline を破損 PNG として拒否する', () => {
  ...
  const result = runPng(createPngFromScanlines(120, 80, Buffer.alloc(expected - 10)));
...
test('行が欠けた PNG を破損 PNG として拒否する', () => {
  const result = runPng(createPngFromScanlines(120, 80, Buffer.alloc((120 * 4 + 1) * 79)));
...
test('色変化が多い最大寸法を時間上限内で判定する', { timeout: 15_000 }, () => {
  ...
  const result = runPng(createPngFromScanlines(width, height, scanlines, 2), 8_000);
