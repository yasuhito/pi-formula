...
function inspect(output, expected) {
  ...
  const finalPlaceholders = number(final, /cells\.placeholders=(\d+)/u);

  if (frames.length < 2) return "読み取りごとのフレームが記録されませんでした";
  if (textFrames.length === 0)
    return "未完了本文が文字セルにあるフレームを確認できませんでした";
  ...
  if (virtualImages !== expected || finalPlaceholders < expected)
    return (
      `最終フレームの表示数式 ${expected} 件に対して ` +
      `仮想配置 ${virtualImages} 件、placeholder ${finalPlaceholders} セルです`
    );
  return {
...
function run(tool, expected, configHome) {
  ...
  const inspected = inspect(result.stdout, expected);
