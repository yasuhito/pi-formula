...
function assistantText(session) {
  ...
  const assistant = records.filter((record) =>
    record.type === 'message' && record.message?.role === 'assistant'
  ).at(-1);
  if (!assistant || assistant.message.stopReason !== 'stop') {
    throw new Error('最後の assistant message が完了していません');
  }
  if (!Array.isArray(assistant.message.content)) throw new Error('assistant content が配列ではありません');
...
function verifyEcho(corpus, session) {
  ...
  if (actual !== expected) {
    let mismatch = 0;
    while (mismatch < actual.length && mismatch < expected.length
      && actual[mismatch] === expected[mismatch]) mismatch += 1;
    throw new Error(
