...
Pi の公開 Markdown transformer と TUI renderer を使い、Issue 26 の未完了本文を文字セルとして順に描いた後、確定本文を画像経路で描く。`vt-pty --frames` が記録したすべてのフレームで本文セルの APC 断片が0件であることを検査する。最終フレームでは、表示数式と storage 付き仮想配置の数が一致し、placeholder セルがあることも検査する。

この検査により、ADR 0001 が定める「ストリーミング中はテキスト経路、確定後は画像経路」という現行経路を時系列で確認する。未完了本文のフレームでは仮想配置を作らず、確定フレームで3件の仮想配置と対応する placeholder が揃い、途中の APC 断片は観測されない。確定待ちを外す変更は ADR 0001 の見直しと別の実測を要するため、この検査では行わない。
