...
ハーネスは実行ごとに一意な headless 出力を作る。作成コマンドの前に後片付けを有効にするため、作成が副作用後に失敗しても出力を削除する。要求した1920px幅と計画高がmonitorへ適用されたことを確認する。Ghosttyは`hl.dsp.exec_cmd`の`workspace`と`monitor`の`silent`規則で、その出力へ作成時から割り当てる。`no_initial_focus` も指定する。後追い移動はしない。起動後は、Ghostty の monitor ID と実行前後の active window が変わっていないことを検査する。条件を満たさなければキャプチャせず失敗する。
